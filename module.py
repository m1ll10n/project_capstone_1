...
def eda(df):
    """Perform EDA on dataset. Checking for null values in target column
...
def plot_data(df):
    """Plotting Time-Series Data for Number of New Cases
...
def data_mms(WINDOW_SIZE, data):
    """Scaling data and split into feature variable(X) and target variable(y)
...
def predict_score(data, prediction):
    """Scores model accuracy against prediction and data
...
def test_prepare(TEST_PATH, WINDOW_SIZE, df, data, mms):
    """Preparing test dataset for predictions using Sequential model
